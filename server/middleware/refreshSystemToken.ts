...
export default function refreshSystemToken({ hmppsAuthClient }: Services): Router {
  ...
  router.use(async (req, res, next) => {
    ...
    try {
      req.session.systemToken = await hmppsAuthClient.getSystemClientToken(res.locals.user.username)
