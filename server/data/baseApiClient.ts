...
export default class BaseApiClient extends RestClient {
  constructor(
    ...
    _config: typeof config.apis.welcome,
  ...
  protected apiCall<
    ReturnType extends object | string | void,
  ...
  >({
    ...
    loggerMessage?: (p: Parameters, d: Data) => string
    options?: {
    ...
    return async (token: string, parameters: Parameters = {} as never, data: Data = undefined): Promise<ReturnType> => {
      if (loggerMessage) {
        logger.info(loggerMessage(parameters, data))
      }
      try {
